...
?dnorm
for(i in 1:500){
  dpost1[i] <- function(x){dnorm(x, mean=mu6, sd=sigma[1])}
  curve(dpost1, from=3, to=6, xlab="parameter", ylab="posterior density", lty=1)
  dpost2[i] <- function(x){dnorm(x, mean=mu4, sd=sigma[2])}
  curve(dpost2, from=3, to=6, xlab="", ylab="", lty=2, add=TRUE)
  dpost3[i] <- function(x){dnorm(x, mean=mu2, sd=sigma[3])}
